...
def build_smoke_report(base_url: str | None = None) -> dict:
    target = (base_url or os.getenv("SENTINEL_API_BASE_URL") or "http://127.0.0.1:8000").rstrip("/")
    checks = {
        "GET /health": _check_endpoint(target, "/health", 200),
        "GET /internal/manifest": _check_endpoint(target, "/internal/manifest", 200),
        "GET /internal/env/source": _check_endpoint(target, "/internal/env/source", 200),
    ...
    return {
        "base_url_configured": bool(target),
        "checks": checks,
        "smoke_test_verdict": "pass" if ready else "fail",
    }


def format_smoke_report(report: dict) -> str:
    lines = ["Sentinel Alpha Smoke Test v1.5"]
    lines.append(f"base url configured: {str(report['base_url_configured']).lower()}")
    for name, result in report["checks"].items():
        status = "pass" if result["ok"] else "fail"
        status_code = result["status_code"]
        lines.append(f"{name}: {status} (status={status_code})")
