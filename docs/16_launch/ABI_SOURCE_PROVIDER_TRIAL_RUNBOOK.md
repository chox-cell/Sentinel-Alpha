...
- v11.8 Blockscout Base endpoint source pack: `docs/16_launch/BLOCKSCOUT_BASE_ENDPOINT_SOURCE_PACK.md` (docs/test only; **no** candidate selected in v11.8 revision). **Read this source pack and complete one-candidate selection per its rules before Blockscout endpoint validation** (in addition to the validation plan gate).
...
- Blockscout Base endpoint source pack read when Blockscout is the chosen next path: `docs/16_launch/BLOCKSCOUT_BASE_ENDPOINT_SOURCE_PACK.md` (v11.8; **exactly one** candidate must be selected before validation per plan §5a).
